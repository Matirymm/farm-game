...
class CropSpec {
  final String id, nameAr, emoji;
  final int unlockLevel, seedCost, growSeconds, sellPrice, xp;
  const CropSpec(this.id, this.nameAr, this.emoji, this.unlockLevel,
      this.seedCost, this.growSeconds, this.sellPrice, this.xp);
}

class AnimalSpec {
  final String id, nameAr, emoji, productId;
  final int unlockLevel, price, produceSeconds;
  final Map<String, int> feed; // cropId -> كمية
  const AnimalSpec(this.id, this.nameAr, this.emoji, this.unlockLevel,
      this.price, this.feed, this.produceSeconds, this.productId);
}

class ProductSpec {
  final String id, nameAr, emoji;
  final int sellPrice;
  const ProductSpec(this.id, this.nameAr, this.emoji, this.sellPrice);
}
...
const animals = <String, AnimalSpec>{
  'chicken':
      AnimalSpec('chicken', 'دجاجة', '🐔', 3, 20, {'wheat': 2}, 15, 'egg'),
...
String emojiOf(String id) => crops[id]?.emoji ?? products[id]?.emoji ?? '❓';
